...
`useSessionLifecycle` is the only presentation controller that creates, restores, or mutates API sessions. Every accepted action replaces the displayed revision and snapshot with the API response; revision conflicts trigger an authoritative session reload instead of replaying a stale mutation. Manual-note taps are the one exception to waiting for a round trip: a transient cell-local draft renders immediately and debounces rapid edits into one `set-notes` request. Each cell serializes note requests; input received while one request is in flight remains visible and is sent as the next complete set after the response, so an older authoritative snapshot cannot erase newer input. A settled draft disappears when the authoritative response arrives, while conflict or failure recovery replaces it with the recovered snapshot. The draft is never persisted and does not own revision, history, or validation semantics. `useGameTimer` and `useBoardNavigation` consume authoritative session snapshots but own only elapsed-time display, pause visibility, selection, focus, notes mode, and keyboard or pointer routing.
...
- Candidate adoption MUST be one backend action; the browser MUST NOT copy the derived grid or split adoption across cell requests.
