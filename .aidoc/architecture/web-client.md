...
Number-pad buttons use canonical click activation for mouse and keyboard, while touch and pen commit on pointer release so a second quick tap does not wait for a delayed compatibility click. The first click in the short post-pointer window is suppressed unconditionally because mobile browsers can emit compatibility clicks with no pointer type, detail, or coordinates; CSS `touch-action: manipulation` also removes double-tap gesture ambiguity. Each completed activation therefore produces exactly one note toggle across input methods.
