...
Input handling is split into three layers. `useBoardNavigation` defines device-independent commands and UX state transitions such as enter digit, erase, change mode, and move selection. `ExactActivationButton` is a thin input adapter that normalizes button events into one semantic activation: mouse and keyboard use canonical click activation, touch commits on `touchend`, and pen commits on pointer release. Using the touch event itself avoids depending on mobile browsers to populate Pointer Events metadata or deliver a prompt compatibility click when several digits are tapped quickly. The adapter suppresses the first click in the short post-release window unconditionally because mobile browsers can emit compatibility clicks with no pointer type, detail, or coordinates; CSS `touch-action: manipulation` also removes double-tap gesture ambiguity. `useSessionLifecycle` then serializes resulting gameplay actions independently of their input source. Device heuristics therefore cannot change Sudoku semantics, note drafting, revisions, or history.
