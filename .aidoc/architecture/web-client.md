...
Input handling is split into three layers. `useBoardNavigation` defines device-independent commands and UX state transitions such as enter digit, erase, change mode, and move selection. `ExactActivationButton` is a thin input adapter that normalizes button events into one semantic activation: mouse and keyboard use canonical click activation, while touch and pen commit on pointer release so a second quick tap does not wait for a delayed compatibility click. The adapter suppresses the first click in the short post-pointer window unconditionally because mobile browsers can emit compatibility clicks with no pointer type, detail, or coordinates; CSS `touch-action: manipulation` also removes double-tap gesture ambiguity. `useSessionLifecycle` then serializes resulting gameplay actions independently of their input source. Device heuristics therefore cannot change Sudoku semantics, note drafting, revisions, or history.
