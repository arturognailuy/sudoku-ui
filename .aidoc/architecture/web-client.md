...
The browser owns only presentation concerns such as selection, keyboard focus, pause visibility, elapsed-time display, theme, and preferences. Theme preference is persisted as `system`, `light`, or `dark`; System follows `prefers-color-scheme` changes, while the resolved theme selects one complete semantic token set for every surface and gameplay state. Automatic candidates are an opt-in browser display preference over authoritative `snapshot.candidates`; enabling or disabling the preview never creates a mutation or derives candidates locally. A first digit edit while Notes and Candidates are both active submits one backend `adopt-candidates-as-notes` action, then returns the browser to manual-note display after success. A small local active-game record stores the opaque API session ID, optional known difficulty, and timer presentation state so refresh can request the authoritative snapshot again; it never stores puzzle values or history. The welcome surface also reads the API's bounded session summaries so a player can explicitly continue or discard durable games. Opening a listed or imported session deliberately labels it as a saved puzzle when its original difficulty is not part of the API contract rather than inventing metadata.

## What the Client Contains

`SudokuApiClient` is the narrow transport boundary. Its request and response types mirror the canonical OpenAPI 3.1.1 contract in `gnailuy/sudoku/api/openapi.yaml`; transport failures become `SudokuApiError` values rather than leaking fetch details through the component tree. Session portability stays byte-oriented: export downloads the API's versioned media type without interpreting it, and import posts the selected file directly for server validation. Discard calls the authoritative delete endpoint only after an explicit confirmation.
...
- Session lists MUST contain summaries only; continuing MUST fetch the chosen authoritative snapshot before showing a board.
- Import/export MUST treat the versioned session document as opaque bytes, and discard MUST require an explicit confirmation.
