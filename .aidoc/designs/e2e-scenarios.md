...
### Input-method gameplay matrix

**Action:** Run the same black-box gameplay sequence three times, with each run restricted to one input method: keyboard, mouse, or touchscreen. Start a game, select an editable cell, enable Notes, add and remove one individual note, rapidly add adjacent notes 4 and 5 under delayed responses, enable Candidates, adopt and edit the displayed candidate set, return to value mode, and enter both an invalid and a valid value.

**Expected:** All three independent runs produce the same state and API actions without borrowing another input method. Keyboard and mouse use canonical click activation. Touch and pen commit on pointer release, while the first click inside the short post-pointer window is suppressed even when the browser omits pointer type, click detail, and coordinates. Each completed activation toggles exactly once: an individual note persists, a second activation removes it, and rapid notes 4 and 5 both appear immediately. Candidate adoption preserves later edits, invalid values receive authoritative invalid styling, valid values remain normal, and empty, one-digit, and multi-digit note sets use the same wire action.
