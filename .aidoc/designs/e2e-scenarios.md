...
**Action:** Select an empty editable cell, enable Notes, and rapidly add adjacent notes 4 then 5 through a real touch input path without waiting for a render between taps. Repeat the rapid pair through keyboard and mouse while a request is deliberately delayed, then rapidly edit notes during candidate adoption.

**Expected:** Keyboard and mouse use canonical click activation. Touch and pen commit on pointer release, while any delayed compatibility click is suppressed as the duplicate of that release. Each completed activation toggles exactly once, notes 4 and 5 both appear immediately, and the first `set-notes` request carries `[4, 5]`. Rapid keyboard and mouse pairs produce the same exact state, and all later additions and removals remain visible while a request is in flight. Each accepted request creates one authoritative revision, while conflict or failure recovery replaces the transient draft with the backend snapshot. Empty, one-digit, and multi-digit note sets use the same wire action.
