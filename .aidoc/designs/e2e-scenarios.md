...
**Action:** Select an empty editable cell, enable Notes, and toggle notes through keyboard, mouse click, and a real touch input path. Then tap several number-pad digits faster than the debounce window while a request is deliberately delayed, and rapidly add one note and remove multiple notes through the number pad.

**Expected:** Keyboard, mouse, touch, and pen use the same canonical button activation and each completed activation toggles exactly once. A pointer-down by itself never mutates notes, so a later compatibility click cannot undo an earlier pointer-specific toggle. Every rapid tap is treated exactly once, every toggle appears immediately, the first `set-notes` request carries its complete sorted set, and all later additions and removals remain visible while that request is in flight. After the first response, a serialized second request carries the latest complete set without an intermediate rollback. Each accepted request creates one authoritative revision, while conflict or failure recovery replaces the transient draft with the backend snapshot. Empty, one-digit, and multi-digit note sets use the same wire action.
