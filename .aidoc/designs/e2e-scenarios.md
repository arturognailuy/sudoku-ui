...
**Action:** Select an empty editable cell, enable Notes, and tap several number-pad digits faster than the debounce window using the browser's touch input path. While that request is deliberately delayed, rapidly add one note and remove multiple notes through the number pad.

**Expected:** Touch and pen input is accepted on pointer down instead of waiting for a synthesized click, and the later compatibility click is ignored so a digit cannot toggle twice. Every rapid tap is therefore treated exactly once, every toggle appears immediately, the first `set-notes` request carries its complete sorted set, and all later additions and removals remain visible while that request is in flight. After the first response, a serialized second request carries the latest complete set without an intermediate rollback. Mouse and keyboard activation retain their normal click path. Each accepted request creates one authoritative revision, while conflict or failure recovery replaces the transient draft with the backend snapshot. Empty, one-digit, and multi-digit note sets use the same wire action.
