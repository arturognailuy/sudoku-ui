...
Starting another puzzle from an active game requires explicit confirmation. The current timer stops while that decision is open, Escape or the safe first action returns to the unchanged board, and only the named confirmation replaces the browser's active-session pointer with a newly created puzzle.
